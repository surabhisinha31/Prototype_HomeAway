...
import {ownerlogin} from './../api/Api';
...
class OwnerSignIn extends Component {
  ...
  render() {
    console.log(" Owner Sign In render");
    return (
    <div className="main-div">
        <TopHeader/>
    <div className="login-div">
        <h1 className="login-header"> Need an account ?
            <a href="ownersignup" className="header-img"> Sign Up</a>
        </h1>
    ...
    <div className="container-fluid" style={{marginTop:"3%",marginLeft:"1%"}} >
          <form className="form-group">
            <div className="field-details">
              <h3 className="header-login1"> Owner Account Login </h3>
              <hr/>
            ...
            <button type="button" onClick={() =>
              (VALIDATION.validateEmail(this.userinfo.username,"Username") && VALIDATION.emptyDate(this.userinfo.password,"Password"))==true?this.props.ownerlogin(this.userinfo):''}
            ...
            <a href="ownersignin" className="header-owner"> Traveler Login</a>
            </div>
          </form>
        </div>
    </div>
...
function matchDispatchToProps(dispatch){
    ...
    return bindActionCreators({ownerlogin: ownerlogin}, dispatch);
}
export default connect(null, matchDispatchToProps)(OwnerSignIn);
